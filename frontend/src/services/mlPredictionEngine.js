...
export const FEATURE_SCALER = {
  means: {
    freq_5: 0.05, freq_10: 0.1, freq_20: 0.2, freq_50: 0.5, freq_100: 0.9829, freq_all: 4.5675,
    delay_head: 92.8397, delay_avg: 93.8828, delay_max: 157.1736, delay_std: 40.2601,
    trend_recent_vs_all: 0.0, trend_10_vs_50: 0.0, trend_20_vs_100: 0.0002,
    pos_head_freq: 0.01, pos_top5_freq: 0.01, pos_top10_freq: 0.01, pos_top20_freq: 0.01,
    shift_freq: 0.0399, weekday_freq: 0.0298, unit_freq: 0.1, decade_freq: 0.1, markov_prob: 0.1
  },
  scales: {
    freq_5: 0.2219, freq_10: 0.3189, freq_20: 0.4501, freq_50: 0.7154, freq_100: 1.0021, freq_all: 3.238,
    delay_head: 89.3737, delay_avg: 73.9108, delay_max: 100.3295, delay_std: 40.2554,
    trend_recent_vs_all: 0.0313, trend_10_vs_50: 0.0285, trend_20_vs_100: 0.0201,
    pos_head_freq: 0.0061, pos_top5_freq: 0.0028, pos_top10_freq: 0.0019, pos_top20_freq: 0.0014,
    shift_freq: 0.012, weekday_freq: 0.0116, unit_freq: 0.0226, decade_freq: 0.0205, markov_prob: 0.0592
  }
};
...
export function extractAmboFeaturesClient(historyDraws, targetLottery = "ciudad", targetShift = "nocturna", targetDate = null) {
  ...
  const cleanTargetShift = (targetShift || 'matutina').toLowerCase().replace('la_', '');
  const cleanTargetLottery = (targetLottery || 'all').toLowerCase();

  // Filtrar historial específico por lotería si no es 'all' y hay muestra suficiente (>= 20)
  const lotHistory = (cleanTargetLottery === 'all')
    ? historyDraws
    : historyDraws.filter(d => (d.lottery || '').toLowerCase() === cleanTargetLottery);
  const activeHistory = lotHistory.length >= 20 ? lotHistory : historyDraws;
  ...
  for (let idx = 0; idx < nActive; idx++) {
    ...
    const dShift = (d.shift || '').toLowerCase().replace('la_', '');
    ...
    if (Array.isArray(d.board)) {
      d.board.forEach((item, posIdx) => {
        ...
        if (!isNaN(amboInt) && amboInt >= 0 && amboInt < 100) {
          ...
          if (dShift === cleanTargetShift) shiftCounts[amboInt]++;
...
export function getMLPredictions(lotteryOrOpts = "all", shift = "auto", topCount = 5, beforeDate = null) {
  ...
  const scoredAmbos = extracted.features.map(item => {
    ...
    for (const [featKey, weight] of Object.entries(FEATURE_WEIGHTS)) {
      const rawVal = f[featKey] || 0;
      const mean = FEATURE_SCALER.means[featKey] ?? 0;
      const scale = FEATURE_SCALER.scales[featKey] || 1;
      const zScore = (rawVal - mean) / scale;
      const term = weight * zScore;
      logit += term;
